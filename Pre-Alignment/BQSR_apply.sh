...
do  
    ...
    then
        ...
        if [ ${i} = 1 ]
        then
            echo ${finalPath}/${filename}_final.bam > ${finalPath}/merge_file.list
        else
            echo ${finalPath}/${filename}_final.bam >> ${finalPath}/merge_file.list
        fi
    fi
...
# multiple bam merge
gatk MergeSamFiles "-Xmx8G -Xmx25G -XX:+UseParallelGC -XX:ParallelGCThreads=4" \
     -I ${finalPath}/merge_file.list \
